...
## 双亲委托
除了顶层的启动类加载器以外，其余的类加载器，在加载之前，都会委派给它的父加载器进行加载。这样一层层向上传递，直到祖先们都无法胜任，它才会真正的加载。Java默认是这种行为，但是如SPI（JDBC驱动加载）、OSGI等并没有使用双亲委托
![avatar](classLoader.jpg)

## 并发
### 锁
| 对比维度          | synchronized（内置隐式锁）                 | Lock（以 ReentrantLock 为例，显式锁）                  |
|---------------|:------------------------------------|:----------------------------------------------|
| 所属层面          | JVM 原生关键字，底层由虚拟机实现                  | JDK 代码层面实现，基于 AQS 框架                          |
| 加解锁方式         | 隐式，进入同步块加锁，退出 / 异常自动释放锁             | 显式，手动调用 lock() 加锁、unlock() 解锁；必须在 finally 中解锁 |
| 可重入性          | 支持可重入，同一线程可多次获取                     | 支持可重入                                         |
| 锁类型（公平 / 非公平） | 只支持非公平锁，无法修改                        | 默认非公平，可构造方法指定公平锁                              |
| 中断等待          | 阻塞后不可中断，线程会一直等待                     | 支持中断：lockInterruptibly() 可响应中断                |
| 超时等待          | 不支持，拿不到锁永久阻塞                        | 支持：tryLock(时间)，超时直接返回，避免死等                    |
| 条件队列          | 只有 1 个 等待队列（wait/notify/notifyAll）  | 可创建 多个 Condition 条件队列，精准唤醒                    |
| 锁升级           | JDK1.6+ 存在锁升级：偏向锁 → 轻量级锁 → 重量级锁，不可逆 | 无锁升级机制，底层直接基于 AQS 实现                          |
| 性能表现          | JDK1.6 优化后性能大幅提升，低竞争下表现优秀           | 高并发、竞争激烈场景下性能略占优势                             |
| 功能丰富度         | 功能简单，满足基础同步需求                       | 功能更强，支持中断、超时、公平锁、多条件队列                        |
| 异常处理          | 代码抛出异常，锁自动释放，不会死锁                   | 异常不会主动释放锁，必须手动 unlock，遗忘会造成死锁                 |
| 使用语法          | 写法简洁，无需额外代码                         | 写法繁琐，需成对编写加锁、解锁逻辑                             |
| 适用场景          | 简单同步场景、业务逻辑简单，优先使用                  | 需要中断、超时、公平锁、精准唤醒等复杂场景                         |
...
##### 参数解释
> * corePoolSize(重要)：任务队列未使用前，最大可以同时运行的线程数量
> * maximumPoolSize(重要)：任务队列中存放的任务达到队列容量的时候，当前可以同时运行的最大线程数
> * workQueue(重要)：用于存放任务的队列，corePoolSize < 当前线程数 且队列未满时，任务会被放入
...
> G1：专为服务器端应用设计，特别适用于具有多核处理器和大内存的机器。它的主要目标是在满足高吞吐量的同时，尽可能缩短垃圾收集造成的停顿时间  
> 过程：
...
>ZGC：一款追求及时低延迟（停顿时间不超过10ms，JDK16后可控知道1ms内）的并发垃圾回收器，停顿时间不会随着堆大小或活跃对象大小而增加，支持堆范围8MB～16TB。其内存布局如下
> ![avatar](zgc_region.png)  
> 过程：
> * 初始标记：仅扫描GC Roots直接引用的对象；需要STW
> * 并发标记：与应用线程并发，便利对象图，用读屏障辅助处理引用变化，通过“着色指针”标记存活对象
> * 初始转移：仅处理GC Roots中指向重分配集内对象的引用，将其“自愈”到新地址（<1ms）；其余对象转移由应用线程在读屏障中协助完成；需要STW
> * 并发重分配：并发移动冲分配集中存活对象到新Region，更新转发表，并并发修正引用；重映射被延迟和冰岛下次并发标记中
